...
- We calculated the .blue[mean price] in our sample: a single number (point estimate)

- But how confident are we that this estimate is close to the .blue[true population value]?

- A **confidence interval** quantifies our uncertainty: it gives a range of plausible values for the parameter

---
### Confidence Intervals

- Our sample mean price was $\bar{x}=1245.43$

--

- Is it plausible that the true average price is 1100? What about 2000?

--

- Suppose we calculated the 90% confidence interval:
$$(1086.64,\; 1404.22)$$

--

- Where do these bounds come from? Two key ideas:

--

  1. The .blue[sampling distribution] of $\bar{X}$ tells us how far a sample mean can reasonably fall from the true mean

--

  2. We .blue[invert] this logic: if $\bar{X}$ is unlikely to be far from $\mu$, then $\mu$ is unlikely to be far from $\bar{X}$

--

- Let's develop each idea step by step
...
\begin{align*}
1-\alpha &= P\!\left(-z_{\frac{\alpha}{2}}<Z<z_{\frac{\alpha}{2}}\right) \\[4pt]
&= P\!\left(-z_{\frac{\alpha}{2}}<\frac{\bar{X}-\mu}{\sigma/\sqrt{n}}<z_{\frac{\alpha}{2}}\right) \\[4pt]
&= P\!\left(-z_{\frac{\alpha}{2}}\cdot\frac{\sigma}{\sqrt{n}}<\bar{X}-\mu<z_{\frac{\alpha}{2}}\cdot\frac{\sigma}{\sqrt{n}}\right) \\[4pt]
&= P\!\left(\bar{X}-z_{\frac{\alpha}{2}}\cdot\frac{\sigma}{\sqrt{n}}< \mu <\bar{X}+z_{\frac{\alpha}{2}}\cdot\frac{\sigma}{\sqrt{n}}\right)
\end{align*}

--

- CLT guarantees $\frac{\bar{X}-\mu}{\sigma/\sqrt{n}} \approx N(0,1)$ for large $n$

--

- **What if $\sigma$ is unknown?** In large samples $s \rightarrow \sigma$, so:
$$\frac{\bar{X}-\mu}{s/\sqrt{n}} \;\approx\; N(0,1)$$
- The standardized mean with $s$ in place of $\sigma$ is still approximately normal
- You may need a slightly larger $n$ to compensate for estimating $\sigma$
...
$$\boxed{\small \left(\bar{x}- z_{\frac{\alpha}{2}}\cdot\underbrace{\frac{s}{\sqrt{n}}}_{SE},\;\; \bar{x}+ z_{\frac{\alpha}{2}}\cdot\frac{s}{\sqrt{n}}\right)}$$


---
### Finding Critical Values

- Suppose confidence level is 99%, so $\alpha=0.01$
- We are looking for $z_{\frac{\alpha}{2}}$ such that:

.learnr-ref[**LearnR:** Finding critical values in normal — use `qnorm()` in R]
...
5. Construct the confidence interval as:
$$\small \left(\bar{x}- z_{\frac{\alpha}{2}}\cdot\frac{s}{\sqrt{n}},\;\; \bar{x}+ z_{\frac{\alpha}{2}}\cdot\frac{s}{\sqrt{n}}\right)$$
$$\small \left(1245.43- 1.65\cdot\frac{961.9}{\sqrt{100}},\;\; 1245.43+ 1.65\cdot\frac{961.9}{\sqrt{100}}\right)$$
...
> .red[**Common mistake:**] Saying "there is a 90% probability the true mean is in the interval." After computing the interval, the true mean is either inside it or not — there is no probability involved. The 90% refers to the .blue[procedure]: if we repeated sampling many times, 90% of the resulting intervals would contain $\mu$.
...
CI $= \left(\bar{x}\pm z_{\frac{\alpha}{2}}\cdot\frac{s}{\sqrt{n}}\right)$ are wider when: 
- Confidence level is higher (99% is wider than 90%)
- When $n$ is small
- When $\sigma$ is large

`r knitr::include_url('https://seeing-theory.brown.edu/frequentist-inference/index.html#section2', height='380px')`
...
.learnr-ref[**LearnR:** Practice — Build a 90% CI for ride-hailing fares and reveal the true mean]

---

### Which critical values to use?

| Scenario | $\sigma$ known? | Sample size | Use |
|:---------|:-:|:-:|:---:|
| $X$ is .blue[normal] | Yes | Any $n$ | **Normal** ($z$) |
| $X$ is .blue[normal] | No | Any $n$ | .blue[**Student's t**] ($t_{n-1}$) |
| $X$ is .blue[not normal] | Yes | $n \geq 30$ | **Normal** ($z$) |
| $X$ is .blue[not normal] | No | $n \geq 40$ | **Normal** ($z$) |
| $X$ is .blue[not normal] | — | $n < 30$ | Unreliable; use bootstrap |

--

**Key intuition:**
- When $X$ is normal and $\sigma$ is unknown, use the .blue[t-distribution] — it accounts for the extra uncertainty from estimating $\sigma$
- When $X$ is not normal, you need large $n$ so the CLT kicks in
- With unknown $\sigma$, you need slightly more data ($n\geq 40$) so that $s \approx \sigma$
- As $n$ grows, $t_{n-1}$ converges to $N(0,1)$, so for large $n$ the choice barely matters
...
.learnr-ref[**LearnR:** Finding critical values in Student's t — compare `qt()` vs `qnorm()`]
...
$$\small \left(\bar{x}- t_{\frac{\alpha}{2},n-1}\cdot\frac{s}{\sqrt{n}},\;\; \bar{x}+ t_{\frac{\alpha}{2},n-1}\cdot\frac{s}{\sqrt{n}}\right)$$
...
.learnr-ref[**LearnR:** Practice with Student T — compute the CI and draw conclusions]
...
```{r, warning=FALSE, fig.height=3.5, out.width='90%', fig.align='center'}
...
# Plotting
plot(x_vals, chi_density, type = "l", xlim = c(0, 25), ylim = c(0, 0.15),
...
- Where $\chi^2_{1-\frac{\alpha}{2},n-1}$ and $\chi^2_{\frac{\alpha}{2},n-1}$ are quantiles of the $\chi^2_{n-1}$ distribution
  - $P(X<\chi^2_{1-\frac{\alpha}{2},n-1})=1-\frac{\alpha}{2}$ and $P(X<\chi^2_{\frac{\alpha}{2},n-1})=\frac{\alpha}{2}$
...
.learnr-ref[**LearnR:** Practice for CI of variance — use `qchisq()` to find critical values and build the interval]
