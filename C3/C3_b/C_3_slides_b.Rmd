...
\begin{align*}
0.95&= P(-1.96<Z<1.96) \\
&= P(z_{\frac{\alpha}{2}}<Z<z_{1-\frac{\alpha}{2}}) \\
&= P\left(z_{\frac{\alpha}{2}}<\frac{\bar{X}-\mu}{\sigma/\sqrt  n}<z_{1-\frac{\alpha}{2}}\right) \\
&= P\left(z_{\frac{\alpha}{2}}\cdot\sigma/ \sqrt n<\bar{X}-\mu<z_{1-\frac{\alpha}{2}}\cdot\sigma/\sqrt  n\right) \\
&= P\left(\mu+z_{\frac{\alpha}{2}}\cdot\sigma/\sqrt  n< \bar{X} <\mu+z_{1-\frac{\alpha}{2}}\cdot\sigma/\sqrt  n\right) \\
\end{align*}
...
4. Find the corresponding critical values $\small z_{\frac{\alpha}{2}}$ and $\small z_{1-\frac{\alpha}{2}}$
  - Critical values are such that $\small P(z_{\frac{\alpha}{2}}<Z<z_{1-\frac{\alpha}{2}})=1-\alpha$
  - .blue[Example]: if confidence level is 95%, $\small z_{\frac{\alpha}{2}}=z_{0.025}=-1.96$ and $\small z_{1-\frac{\alpha}{2}}=z_{0.975}=1.96$
--

5. Construct the confidence interval as:
$$\small \left\{\bar{x}- z_{1-\frac{\alpha}{2}}\cdot\underbrace{\frac{s}{\sqrt{n}}}_{SE},\;\; \bar{x}+ z_{1-\frac{\alpha}{2}}\cdot\frac{s}{\sqrt{n}}\right\}$$
...
- We are looking for $z_{1-\frac{\alpha}{2}}$ such that:

.learnr-ref[**LearnR:** Finding critical values in normal — use `qnorm()` in R]
$$P(z_{\frac{\alpha}{2}}<Z<z_{1-\frac{\alpha}{2}})=0.99$$
```{r, warning=FALSE, fig.height=3, out.width='100%'}
curve(dnorm(x),
      xlim = c(-4, 4),
...
# add the "0.5%" labels on tails
text(-2.9, 0.05, "0.5%", col = "black", cex = 1.5)
text(2.9, 0.05, "0.5%", col = "black", cex = 1.5)
```

--
$$P(Z<z_{0.005})=0.005 \qquad\text{and}\qquad P(Z<z_{0.995})=0.995$$
---
...
$P(Z<z_{1-\frac{\alpha}{2}})=0.995$
$z_{1-\frac{\alpha}{2}}$ is the 99.5% quantile of standard normal $\rightarrow$ $z_{1-\frac{\alpha}{2}}=z_{0.995}=2.58$
...
4. Find the corresponding critical value $z_{1-\frac{\alpha}{2}}$
  - Find $z_{1-\frac{\alpha}{2}}$ such that $P(Z<z_{1-\frac{\alpha}{2}})=0.95$
  - $z_{1-\frac{\alpha}{2}}=z_{0.95}=1.65$
--

5. Construct the confidence interval as:
$$\small \left\{\bar{x}- z_{1-\frac{\alpha}{2}}\cdot\frac{s}{\sqrt{n}},\;\; \bar{x}+ z_{1-\frac{\alpha}{2}}\cdot\frac{s}{\sqrt{n}}\right\}$$
...
CI $= \left\{\bar{x}\pm z_{1-\frac{\alpha}{2}}\cdot\frac{s}{\sqrt{n}}\right\}$ are wider when:
...
.learnr-ref[**LearnR:** Interactive CI simulator — draw samples and see how the interval moves relative to the true mean]
...
$$P(T<t_{1-\frac{\alpha}{2},\underbrace{n-1}_{d.f.}})=1-\frac{\alpha}{2} \qquad\text{and}\qquad P(T<t_{\frac{\alpha}{2},n-1})=\frac{\alpha}{2}$$
...
axis(1, at = c(-2.26, 0, 2.26), padj = 0.75,
     labels = c(expression(t[frac(alpha,2)*","~df]),
                0,
                expression(t[1-frac(alpha,2)*","~df])))
...
$$\small \left\{\bar{x}- t_{1-\frac{\alpha}{2},n-1}\cdot\frac{s}{\sqrt{n}},\;\; \bar{x}+ t_{1-\frac{\alpha}{2},n-1}\cdot\frac{s}{\sqrt{n}}\right\}$$
...
.learnr-ref[**LearnR:** Finding critical values in chi-square — use `qchisq()` to find quantiles for a 90% CI]
