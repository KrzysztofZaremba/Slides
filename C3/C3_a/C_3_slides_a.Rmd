...
#### Expectation
- The estimator: $\hat{\theta}_3 = \frac{2X_1 + X_2}{3}$
...
- How do we know that $\frac{3X_1 + X_2}{4}$ is worse then $\frac{X_1+X_2}{2}$?
...
- Suppose $X_1,X_2,...,X_n$ are **i.i.d** variables drawn **at random** from a distribution with mean $\mu$ and standard deviation $\sigma$
  ...
  - Let $S_n=\sum_nX_n$. Note that: $E[S_n]=n\mu$ and $var(S_n)=\sqrt{n}\sigma$
  - Let $\bar{X}_n=\frac{\sum_nX_n}{n}$ Note that: $E[\bar{X}_n]=mu$ and $var(\bar{X}_n)=\frac{\sigma}{\sqrt{n}}$
  - Let $Z_n=\frac{\bar{X}_n-\mu}{\frac{\sigma}{\sqrt{n}}}$ Note that: $E[Z_n]=0$ and $var(Z_n)=1$

- **Central Limit Theorem** says that **for large n**:

$$S_n \sim \mathcal{N}(\mu,\underbrace{\sqrt n \sigma}_{st.dev.}) \qquad\text{and}\qquad \bar{X}_n \sim \mathcal{N}(\mu,\frac{\sigma}{\sqrt n}) \qquad\text{and}\qquad \bar{Z}_n \sim \mathcal{N}(0,1)$$
- For large samples ( $n>30$ ) sample mean is normally distributed with mean $\mu$ and standard deviation $\frac{\sigma}{\sqrt{n}}$ 

---
### Central Limit Theorem 

- The original distribution of $X_i$ does not matter
- Larger **n**, tighter distribution around the mean 
- Smaller ** $\sigma$ **, tighter distribution around the mean 
`r knitr::include_url('https://seeing-theory.brown.edu/probability-distributions/index.html#section3', height='480px')`
.footmark[
...
--
- $var(\bar{X}_n)=\frac{\sigma}{\sqrt n}=\frac{\sqrt{p(1-p)}}{\sqrt n}=\frac{0.5}{\sqrt n}$
---
### Central Limit Theorem 
...
$$\text{If } X \sim \mathcal{N}(\mu_1, \sigma_1^2) \text{ and } Y \sim \mathcal{N}(\mu_2, \sigma_2^2), \text{ then } X + Y \sim \mathcal{N}(\mu_1 + \mu_2, \sqrt{\sigma_1^2 + \sigma_2^2})$$

's delve into the mathematical transformation and visualize how it unfolds.

---

## From Normal to Chi-Square
- We start with the standard random normal distribution N(0, 1).
- The transformation $\smallX = Z^2$ gives rise to the Chi-Square distribution with 1 degree of freedom $\small\chi^2(1)$.
