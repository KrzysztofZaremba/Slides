...
`r knitr::include_url('https://www.zoology.ubc.ca/~whitlock/Kingfisher/SamplingNormal.htm', height='420px')`

.learnr-ref[**LearnR:** Panels A, B, C — Explore sampling distributions of SD, IQR, and 99th percentile]
...
.learnr-ref[**LearnR:** Example 1 — Simulate the sampling distribution of a single observation]
---
...
.learnr-ref[**LearnR:** Example 2 — Simulate the biased estimator and observe the shift]
---
...
.learnr-ref[**LearnR:** Example 3 — Compare the sample mean's spread to a single observation]
---
...
- Relative efficiency of $\hat\mu_B$ to $\hat\mu_A$: how many times more efficient is B than A?
$$\small Eff(\hat\mu_B,\hat\mu_A)=\frac{Var(\hat\mu_A)}{Var(\hat\mu_B)}=\frac{10\sigma^2/16}{4\sigma^2/16}=\frac{10}{4}=\frac{5}{2}$$
- B is 2.5 times as efficient as A. When $Eff>1$, the first estimator is more efficient
...
- $\small  Var(\hat{\theta}_1) = \sigma^2$

--
#### Example 3: $\hat{\theta}_3= \bar{X} = \frac{\sum{X_i}}{n}$
- $\small  Var(\hat{\theta}_3) = \frac{\sigma^2}{n}$

--
#### Estimator A: $\hat\mu_A= \frac{3X_1 + X_2}{4}$
- $\small  Var(\hat\mu_A) = \frac{9\sigma^2+\sigma^2}{16}=\frac{10\sigma^2}{16}$
...
\begin{align*}
\scriptsize MSE(\hat{\theta})&\scriptsize =E[(\hat{\theta}-\theta)^2] =E[(\hat{\theta}-E(\hat{\theta})+E(\hat{\theta})-\theta)^2]\\
&\scriptsize = E[(\hat{\theta} - E(\hat{\theta}))^2] + 2(\underbrace{E[\hat{\theta} - E(\hat{\theta})]}_{=0})(E(\hat{\theta}) - \theta) + (E(\hat{\theta}) - \theta)^2 \\
&\scriptsize = \text{Var}(\hat{\theta}) + \text{Bias}(\hat{\theta})^2
\end{align*}
...
$$\small S_n \mathrel{\dot\sim} \mathcal{N}\!\left(n\mu,\,\sqrt{n}\,\sigma\right), \quad \bar{X}_n \mathrel{\dot\sim} \mathcal{N}\!\left(\mu,\,\frac{\sigma}{\sqrt{n}}\right), \quad Z_n \mathrel{\dot\sim} \mathcal{N}(0,1)$$
...
`r knitr::include_url('https://seeing-theory.brown.edu/probability-distributions/index.html#section3', height='400px')`
...
.learnr-ref[**LearnR:** DiDi Simulator — Work through the problem analytically, then verify with simulation]
...
.learnr-ref[**LearnR:** Interactive Normal vs Chi-Square — adjust the threshold and see both distributions update]
