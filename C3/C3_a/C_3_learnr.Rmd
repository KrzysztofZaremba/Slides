...
tutorial:
  id: "itam-bf-C3-estimators-ci"
  version: 1.0
---

## Population

We start with a simulated population drawn from an Exponential(2) distribution. This is intentionally **non-normal** — it is right-skewed and bounded below by 0. This lets us see how estimators behave when the population is far from a bell curve.
...
- \( E(\hat{\theta}_1) = E(X_i) = \mu \) → **unbiased**.
...
fluidPage(
  ...
  actionButton("go_t2", "Simulate θ₁ = Xᵢ", class = "btn-primary"),
...
- \( E(\hat{\theta}_2) = \frac{4}{5}\mu \) → **biased** (downward).
- Bias \( = -\frac{1}{5}\mu \).
...
fluidPage(
  ...
  actionButton("go_t3", "Simulate θ₂ = (3X₁ + X₂)/5", class = "btn-primary"),
...
- \( E(\hat{\theta}_3) = \mu \) → **unbiased**.
- Compare the spread of this distribution to Example 1. The sample mean is much more concentrated — it has variance \( \sigma^2/n \) instead of \( \sigma^2 \).
...
observeEvent(input$draw_sample_dist, {
  ...
  output$dist_plot <- renderPlot({
    ...
    # Axis labels for μ ± z*·SE (use actual zcrit, not hardcoded 1.96)
axis(1,
     at = c(-zcrit, 0, zcrit),
     labels = c(
       expression(mu - z[1-alpha/2] %*% frac(s, sqrt(n))),
       expression(mu),
       expression(mu + z[1-alpha/2] %*% frac(s, sqrt(n)))
     ),
     padj = 0.75
...
axis(3,
     ...
     padj = -0.75,
     labels = c(
  expression(bar(x) - z[1-alpha/2] %*% frac(s, sqrt(n))),
  expression(bar(x)),
  expression(bar(x) + z[1-alpha/2] %*% frac(s, sqrt(n)))
)
)
...
Suppose we want to test H₀: μ = 100. Let's see what samples look like when the null is true vs. when it's false.
...
## Sugar sticker z-test

A food company produces snack bars. A bar gets an "excessive sugar" sticker if the mean sugar content exceeds 100g. The population standard deviation is known: σ = 20g. You take a sample of n = 36 bars and observe a sample mean of x̄ = 106.

Test H₀: μ ≤ 100 vs H_A: μ > 100 at the 5% significance level.

```{r sugar-ztest, exercise=TRUE}
# Given
xbar <- 106
mu0  <- 100
sigma <- 20
...
# 2) Find the critical value for a one-sided test at α = 0.05
# z_crit <- qnorm(___)
...
# 2) Critical value (one-sided, α = 0.05)
z_crit <- qnorm(0.95)
z_crit

# 3) Reject H₀?
z > z_crit   # TRUE → reject
...
Suppose H₀: μ = 100 is **true**. We'll generate 1000 samples from N(100, 20) and see how often we'd incorrectly reject.

```{r t123, exercise=TRUE}

# Generate 1000 sample means when the null is TRUE (μ = 100)
means_100 <- replicate(1000, mean(rnorm(36, mean = 100, sd = 20)))
head(means_100)

hist(means_100, main="Sampling Distribution of Mean (μ=100, null is true)", xlab="Sample mean")

# Standardize them under H0: μ₀ = 100
# standardized_means <- (means_100 - 100) / (20 / sqrt(36))
# head(standardized_means)

# How many exceed the critical value (one-sided, α = 0.05)?
...
Now suppose the **true** mean is μ = 110, but we are testing H₀: μ = 100. How often do we **fail** to reject?
...
hist(means_110, main="Sampling Distribution of Mean (true μ=110)", xlab="Sample mean")

# Standardize under H0: μ₀ = 100 (we don't know the true mean in practice!)
...
This exercise demonstrates the link between confidence intervals and hypothesis tests. You'll see that rejecting $H_0$ at significance $\alpha$ is equivalent to the $1-\alpha$ CI **not** containing $\mu_0$.

**Scenario:** A food company claims their cereal boxes contain $\mu_0 = 500$g on average. You sample $n = 40$ boxes and get $\bar{x} = 492$ with $s = 25$.

```{r ex_ci_ht_link, exercise=TRUE}
n <- 40
xbar <- 492
s <- 25
mu0 <- 500
...
# 1) Compute the test statistic for H0: mu = 500 vs H_A: mu != 500
# z_test <- (xbar - mu0) / (s / sqrt(n))
# z_test

# 2) Find the critical values for a two-sided test at alpha = 0.05
...
# 4) Now compute the 95% confidence interval
# ci_lower <- xbar - z_crit * (s / sqrt(n))
# ci_upper <- xbar + z_crit * (s / sqrt(n))
# c(ci_lower, ci_upper)

# 5) Does the CI contain mu0 = 500?
# mu0 >= ci_lower & mu0 <= ci_upper

# Compare: the test rejects <=> the CI does NOT contain mu0
```

Try changing `alpha` to 0.01. Does the conclusion change for both the test and the CI?
...
## Small-Sample t-test: McDonald's vs Burger King

We suspect Burger King might have larger caloric content for kids' meals. Both samples are small, so we use the t-distribution.

|   Sample       |  n  | Sample Mean | Sample SD |
|----------------|-----|-------------|-----------|
| McDonald's     |  9  |    475      |    230    |
| Burger King    | 16  |    520      |    180    |

```{r ex_bk_mcdonalds, exercise=TRUE}
# Data
n_mc <- 9;  xbar_mc <- 475; s_mc <- 230
n_bk <- 16; xbar_bk <- 520; s_bk <- 180

# 1) State hypotheses:
#    H0: mu_BK - mu_MC = 0  (no difference)
#    HA: mu_BK - mu_MC > 0  (BK has more calories)

# 2) Standard error for the difference in means
# SE <- sqrt(s_mc^2 / n_mc + s_bk^2 / n_bk)
# SE

# 3) Test statistic
# t_stat <- (xbar_bk - xbar_mc - 0) / SE
# t_stat

# 4) Degrees of freedom (simple approximation)
# df <- min(n_mc - 1, n_bk - 1)
# df

# 5) p-value (one-sided, upper tail)
# p_val <- 1 - pt(t_stat, df)
# p_val

# 6) Try the exact (Welch) degrees of freedom:
# df_welch <- (s_mc^2/n_mc + s_bk^2/n_bk)^2 /
#             ((s_mc^2/n_mc)^2/(n_mc-1) + (s_bk^2/n_bk)^2/(n_bk-1))
# df_welch
# 1 - pt(t_stat, floor(df_welch))
```

At which significance levels do you reject? Does the choice of df formula matter here?
...
# Load your listing data — update the path to match your local directory
...
## Paired or Unpaired? Classification Practice

For each scenario, decide whether the data is **paired** or **unpaired** (two independent samples). Type your answer and reasoning.

**Scenario 1:** A retailer measures daily sales at 50 stores. For each store, they record sales on a regular Tuesday and sales on a promotional Tuesday (same store, two time periods).

```{r ex_pair_q1, exercise=TRUE}
# Is this paired or unpaired? Why?
# answer_1 <- "paired"  # or "unpaired"
...
**Scenario 2:** A company recruits 80 new employees. 40 go through the old training program, 40 go through the new one. After 3 months, they compare performance scores.

```{r ex_pair_q2, exercise=TRUE}
# Is this paired or unpaired? Why?
# answer_2 <- "paired"  # or "unpaired"
...
**Scenario 3:** An economist compares the price of the same 25 grocery items at Walmart vs Costco.

```{r ex_pair_q3, exercise=TRUE}
# Is this paired or unpaired? Why?
# answer_3 <- "paired"  # or "unpaired"
...
**Scenario 4:** A pharmaceutical company tests a new drug. 100 patients receive the drug, 100 different patients receive a placebo.

```{r ex_pair_q4, exercise=TRUE}
# Is this paired or unpaired? Why?
# answer_4 <- "paired"  # or "unpaired"
...
So far you have computed test statistics and p-values by hand. In practice, R has a built-in `t.test()` function that does everything for you. Let's learn to use it.

```{r setup-ttest-demo, include=FALSE}
set.seed(42)
group_A <- rnorm(30, mean = 52, sd = 10)
group_B <- rnorm(35, mean = 48, sd = 12)
```

**Part a)** One-sample t-test: test whether `group_A` has mean equal to 50.

```{r ex_ttest_one, exercise=TRUE, exercise.setup="setup-ttest-demo"}
# One-sample test: H0: mu = 50 vs HA: mu != 50
# t.test(group_A, mu = 50)

# For a one-sided test (HA: mu > 50):
# t.test(group_A, mu = 50, alternative = "greater")
```

**Part b)** Two-sample t-test: test whether `group_A` and `group_B` have the same mean.

```{r ex_ttest_two, exercise=TRUE, exercise.setup="setup-ttest-demo"}
# Two-sample test: H0: mu_A = mu_B vs HA: mu_A != mu_B
# t.test(group_A, group_B)

# Compare: what does the output give you?
# - test statistic (t)
...
**Part c)** Paired t-test: suppose both groups represent the same 30 subjects measured before and after a treatment.

```{r ex_ttest_paired, exercise=TRUE, exercise.setup="setup-ttest-demo"}
before <- group_A[1:30]
after  <- group_B[1:30]

# Paired test: H0: mu_diff = 0 vs HA: mu_diff != 0
# t.test(before, after, paired = TRUE)

# This is equivalent to a one-sample test on the differences:
# diffs <- before - after
# t.test(diffs, mu = 0)
```
