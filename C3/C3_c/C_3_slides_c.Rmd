...
\alpha = P(\text{Type I error}) 
   = \underbrace{P_{H_0}(\bar{X} > 105.483)}_{\text{Pr. if null is true}} 
   = P_{H_0}\!\left( \frac{\bar{X} - 100}{20/\sqrt{36}} > 
       \frac{105.483 - 100}{20/\sqrt{36}} \right)$$

$$\small 
\overset{\text{(CLT)}}{=} P(Z > 1.645) 
   = 1 - \underbrace{\Phi(1.645)}_{\text{Normal CDF}}$$
...
- If I take 10000 samples from distribution with true mean>10000, what share of sample means will fall below the rejection region?
--
...
### A/B Test Power: Professional Photos and Weekly Revenue
...
- Provide photoghraphy to treamtent
...
- If treatment group earns more revenue →  
  we know it’s **caused by the professional photos**.
