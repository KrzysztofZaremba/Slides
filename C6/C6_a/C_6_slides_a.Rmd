...
- How strong is "memory" in the data?
...
--
- Is there a way to transform the data to remove the trend?
...
- First differences approximate how much data grows in each period
...
- Let $\sigma_h$ be the h-step forecast error sd
- Let $k$ be the number of seasonal cycles in the forecast prior to forecast time
  - If it's the first January since time T, no full cycle has passed k=0, k+1=1
  - If it's the second January since time T, 1 full cycle has passed k=1, k+1=2
...
- Generally, average value across 20 years is not a good prediction
...
- Test statistic and the null distribution:
  $$\small T_{test}=\frac{\bar e-0}{\frac{\hat{\sigma}_e}{\sqrt n}} \sim t_{n-1}$$
...
- Answers the question:
  - on average, my forecast is x% wrong (deviates x% from true value)
...
What will be the marriage rate in January 2023 (t_{max}+1)?

- What is my $S_{t_{max}+1}$

--
  - $S_{t_{max}+1}$ for January is:  0.865

--
- What is my $T_{t_{max}+1}$?
...
--
- What is my $C_{t_{max}+1}$?

--
  - Hardest to predict
  - Assume it's the same as last available one: $C_{t_{max}-6}=1.0876$

--
- What is my $R_{t_{max}+1}$?

--
  - We don't expect anything crazy to happen so $R_{t_{max}+1}=1$

--
  - Putting it all together:
  $$\hat{y}_{t_{max}+1}=S_{t_{max}+1}T_{t_{max}+1}C_{t_{max}+1}R_{t_{max}+1}=35859.83$$
...
$$\hat{T}_{t_{max}+1} \pm t_{\alpha/2,n-2}\sqrt{\hat{\sigma_T}^2(1+\frac{1}{n}+\frac{(t_{max}+1-\bar t)^2}{\sum^n_{t=1}(t-\bar t)^2})}$$
  - t_{max} is the number of the last observation
  - $T_{t_{max}+1}$ is the trend prediction 
  - $\hat{\sigma}_T=\frac{SSE}{n-2}$ is the st.dev of residuals from the linear regression.
...
$$CI_{95}=(31187*\underbrace{1.0876}_{C_{t_{max}+1}}*\underbrace{0.865}_{S_{t_{max}+1}}, 45050*\underbrace{1.0876}_{C_{t_{max}T+1}}*\underbrace{0.865}_{S_{t_{max}+1}})=(29339.92, 42381.87)$$
