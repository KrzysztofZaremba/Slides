...
beta1 = cov(ceosal1$roe, ceosal1$salary) / var(ceosal1$roe)
beta0 = mean(ceosal1$salary) - beta1 * mean(ceosal1$roe)
beta1
beta0
...
# Interpretation:
# Each additional percentage point of ROE is associated with about $18,500 more
# in CEO salary (since salary is in thousands).
# The R-squared is very low, so ROE alone is a poor predictor of CEO pay.
# The board should NOT set pay based on ROE alone.
```
...
# Also verify: 1 - SSE/SST
1 - SSE / SST

# Interpretation:
# R-squared shows the fraction of variation in house prices explained by
# square footage alone. Square footage is a meaningful predictor of price,
# but other factors (location, condition, lot size) also matter.
```
...
# Original regression (salary in thousands)
reg_original = lm(salary ~ years, data = mlb1)
summary(reg_original)

# Rescaled regression (salary in actual dollars)
mlb1$salary_dollars = mlb1$salary * 1000

reg_rescaled = lm(salary_dollars ~ years, data = mlb1)
summary(reg_rescaled)

# Comparison:
# - The intercept is multiplied by 1000 (salary went from thousands to dollars)
# - The slope is multiplied by 1000 (each year's effect is now in dollars, not thousands)
# - R-squared does NOT change (scaling does not affect correlation)
# - t-statistic does NOT change (the ratio beta1/se(beta1) is invariant to scaling)
# Conclusion: scaling does NOT affect our substantive conclusions.
```
...
# Step 0: fit the regression
reg = lm(wage ~ exper, data = wage1)
summary(reg)
...
# Step 4: variance of beta1
var_beta1 = sigma2_hat / Sxx
var_beta1

# Step 5: standard error of beta1
se_beta1 = sqrt(var_beta1)
se_beta1

# Verify: this should match the Std. Error for exper in summary(reg)
# Interpretation: The SE tells us how precisely we estimate the effect
# of one more year of experience on hourly wages.
```
...
beta1 = coef(reg)["female"]
se_beta1 = summary(reg)$coefficients["female", "Std. Error"]

# Step 3: Compute t-statistic (one-sided test: H1: beta1 < 0)
t_stat = beta1 / se_beta1
t_stat

# Step 4: p-value for one-sided test (left tail)
n = nrow(wage1)
p_value = pt(t_stat, df = n - 2)
p_value

# Interpretation:
# The coefficient on female is negative and highly significant.
# Women earn significantly less per hour than men.
# The p-value is extremely small, so we reject H0.
# The company should absolutely launch a pay equity review.
```
...
# Re-define needed values from previous steps
beta1=101.550
se_beta1=5.575
alpha=0.01
t_critical=qt(1-alpha/2, df=length(cps$earnwk)-2)
...
# Step 1: Run the regression
reg = lm(wage ~ tenure, data = wage1)
summary(reg)

# Step 2: Extract coefficient and SE
beta1 = coef(reg)["tenure"]
se_beta1 = summary(reg)$coefficients["tenure", "Std. Error"]
...
# Step 4: Construct the interval manually
lower = beta1 - t_critical * se_beta1
upper = beta1 + t_critical * se_beta1
c(lower, upper)

# Step 5: Verify with confint()
confint(reg, "tenure", level = 0.95)

# Interpretation:
# We are 95% confident that each additional year of tenure adds between
# [lower] and [upper] dollars to the hourly wage.
# Management can use this range to calibrate retention bonuses.
```
...
# Step 1: Run the regression
reg = lm(rent ~ avginc, data = rental)
summary(reg)

# Step 2: Extract pieces for manual computation
e = residuals(reg)
n = nrow(rental)
x = rental$avginc
mx = mean(x)
...
# Step 3: Prediction at avginc = 10
x0 = 10
beta0 = coef(reg)[1]
beta1 = coef(reg)[2]
yhat = beta0 + beta1 * x0
...
# Step 4: 95% CI for the mean response (manual)
alpha = 0.05
t_critical = qt(1 - alpha/2, df = n - 2)

margin_of_error = t_critical * sigma_hat * sqrt(1/n + (x0 - mx)^2 / Sxx)

lower = yhat - margin_of_error
upper = yhat + margin_of_error
c(lower, upper)

# Step 5: Verify with predict()
newdata = data.frame(avginc = 10)
predict(reg, newdata, interval = "confidence", level = 0.95)

# Interpretation:
# We are 95% confident that the average rent in cities with avginc = $10,000
# falls between [lower] and [upper]. The company should target this range.
```
...
# Step 3: Prediction at sqrft = 2000
x0 = 2000
beta0 = coef(reg)[1]
beta1 = coef(reg)[2]
yhat = beta0 + beta1 * x0
...
# Step 4: 95% Prediction Interval (manual)
alpha = 0.05
t_critical = qt(1 - alpha/2, df = n - 2)

# NOTE: prediction interval has the extra "1 +" under the square root
margin_of_error = t_critical * sigma * sqrt(1 + 1/n + (x0 - mx)^2 / Sxx)
...
# Step 5: Verify with predict()
newdata = data.frame(sqrft = 2000)
predict(reg, newdata, interval = "prediction", level = 0.95)

# Interpretation:
# We are 95% confident that THIS PARTICULAR house of 2000 sq ft will sell
# for between [lower] and [upper] thousand dollars.
# The PI is wider than a CI because it accounts for individual-level uncertainty.
```
...
plot(reg$fitted.values, reg$residuals,
     ...
     main = "Residual Plot: wage ~ exper",
     pch = 16, col = "steelblue")
abline(h = 0, col = "red", lty = 2)

# Interpretation:
# The scatter plot may show that wages increase with experience but at a
# decreasing rate (diminishing returns). The residual plot may show curvature,
# suggesting the linear model is NOT fully appropriate.
# A quadratic term (exper + exper^2) or log(wage) may be more suitable.
```
...
plot(reg$fitted.values, reg$residuals,
     xlab = "Fitted Values", ylab = "Residuals",
     main = "Checking Homoskedasticity: price ~ sqrft",
     pch = 16, col = "steelblue")
abline(h = 0, col = "red", lty = 2)

# Step 3: Also plot residuals vs the predictor directly
plot(hprice1$sqrft, reg$residuals,
     xlab = "Square Footage", ylab = "Residuals",
     main = "Residuals vs Square Footage",
     pch = 16, col = "steelblue")
abline(h = 0, col = "red", lty = 2)

# Interpretation:
# The residual spread increases with square footage (fan shape).
# This is HETEROSKEDASTICITY -- larger houses have more variable prices.
# Standard OLS standard errors are NOT reliable here.
# We should use robust (heteroskedasticity-consistent) standard errors.
```
...
# Step 1: Run the regression
reg = lm(price ~ lotsize, data = hprice1)
summary(reg)

resid_price = reg$residuals

# Step 2: Histogram of residuals
hist(resid_price, breaks = 30, main = "Histogram of Residuals: price ~ lotsize",
     xlab = "Residuals", col = "lightblue")

# Step 3: QQ plot
qqnorm(scale(resid_price), main = "QQ Plot of Residuals")
qqline(scale(resid_price), col = "red", lwd = 2)

# Step 4: Jarque-Bera test
library(tseries)
jarque.bera.test(resid_price)

# Interpretation:
# The histogram is likely right-skewed (house prices are bounded below by 0).
# The QQ plot shows departure from the line in the tails.
# The JB test will likely reject normality (small p-value).
# With a small sample (88 obs), the normality violation matters more.
# Consider log(price) for a better-behaved residual distribution.
```
...
# Step 1: Run the regression and ANOVA
reg = lm(salary ~ years, data = mlb1)
anova(reg)

# Step 2: Also look at summary for context
summary(reg)

# Interpretation:
# The ANOVA F-statistic tests H0: beta1 = 0 (years has no effect on salary).
# Look at the p-value:
# - If p < 0.05, years in the league is a statistically significant predictor.
# - However, check R-squared: even if significant, years may explain only
#   a modest fraction of the variation in baseball salaries.
# The team should also consider batting average, home runs, and other
# performance metrics beyond years alone to understand player compensation.
```
