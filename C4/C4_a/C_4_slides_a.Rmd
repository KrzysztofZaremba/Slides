...
---
Let's go back to our reression line
```{r, warning=FALSE, fig.height=5, out.width='100%', message=FALSE}
...
- $\beta_0$ and $\beta_1$ are estimators
...
$$\begin{align*}
\text{Var}(\hat{\beta}_0) & =\text{Var}(\bar{y}-\beta_1\bar{x})=\text{Var}(\bar{y})+\bar{x}^2\text{Var}(\hat{\beta}_1)-2\bar{x}\underbrace{cov(\bar{y},\hat{\beta}_1)}_{0} \\
...
$$\hat{\beta_1} \sim N\left(\beta_1,\frac{\sigma^2}{S_{xx}}\right) \qquad and  \qquad \hat{\beta_0} \sim N\left(\beta_0,\sigma^2({\frac{1}{n}+\frac{\bar{x}^2}{S_{xx}}})\right)$$
--
...
**Average response**:
What would be average number of rides on days with temperatyre of 30C?
...
- You can have to types of confidence intervals
...
What's the variance?
$$var(\hat{y}|x=x_0)=Var(\hat{\beta_0}+\hat{\beta_1}x_0)+Var(u_i)=\sigma^2(1+{\frac{1}{n}+\frac{(x_0-\bar{x})^2}{S_{xx}}})$$
