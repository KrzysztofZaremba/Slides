...
--
3. She wants to know: how the bike use will change when the temperature increases by 1 degree
...
<div class="learnr-ref">LearnR: 1.1–1.4 Running a Regression (eBay, House Prices, BMI, CEO Pay)</div>
...
- Residual term (e) here reflects both uncertainty about parameters and the random part present in population model
...
$$(\hat{\beta}_0,\hat{\beta}_1)=argmin_{b_0,b_1} SSE=argmin_{b_0,b_1}\sum^n_{i=1}e_i^2$$
--
$$\begin{align*}
...
$$(\hat{\beta}_0,\hat{\beta}_1)=argmin_{b_0,b_1} SSE=argmin_{b_0,b_1}\sum^n_i\left( y_i - (b_0 + b_1 x_i) \right)^2$$
...
$$\small \hat{\beta}_0=\frac{\sum y_i-\hat{\beta}_1\sum x}{n}=\bar{y}-\hat{\beta}_1\bar{x}$$
...
Source: [Interactive Visualization](https://observablehq.com/@yizhe-ang/interactive-visualization-of-linear-regression)
...
<div class="learnr-ref">LearnR: 2.1–2.3 Decomposition & R-squared (House Prices, eBay, Firm Size)</div>
...
p1 <- ggplot(data_high, aes(x, y)) +
  ...
  theme_xaringan()+xlim(-2,2)+ylim(-3,3)
...
### How much of bike usage does the temperature explain?
...
```{r, echo=TRUE, warning=FALSE, fig.height=6, out.width='80%'}
...
<div class="learnr-ref">LearnR: 3.1–3.2 Scaling Variables (Family Income, Baseball Salaries)</div>
...
```{r, echo=TRUE, warning=FALSE, fig.height=5, out.width='100%', message=FALSE}
...
ggplot(amazon_data, aes(x = rating, y = returned)) +
  ...
  theme_xaringan()+ #increase size of axis labels and ticks and all fonts
...
<div class="learnr-ref">LearnR: 5.1–5.2 Hypothesis Testing (Healthcare Costs, Gender Pay Gap)</div>
...
```{r echo=FALSE, results='asis'}
cat(paste0("$$ \\hat{y}_i = ", round(intercept, 2), " + ", round(slope, 2), "x_i $$"))
...
<div class="learnr-ref">LearnR: 4.1–4.3 Standard Errors (House Prices, Rescaling, Wage-Experience)</div>
...
<div class="learnr-ref">LearnR: 4.2 Standard Errors: Effect of Rescaling Y</div>
...
$$\hat{\beta}_1 \sim N\left(\beta_1,\frac{\sigma}{\sqrt{S_{xx}}}\right) \qquad and  \qquad \hat{\beta}_0 \sim N\left(\beta_0,\sigma\sqrt{({\frac{1}{n}+\frac{\bar{x}^2}{S_{xx}}}})\right)$$
--
...
$$\small  \hat{\beta}_1 \;\;\overset{d}{\longrightarrow}\;\; N\left(\beta_1,\frac{\sigma}{\sqrt{S_{xx}}}\right)  and  \qquad \hat{\beta}_0 \;\;\overset{d}{\longrightarrow}\;\; N\left(\beta_0,\sigma\sqrt{({\frac{1}{n}+\frac{\bar{x}^2}{S_{xx}}}})\right)$$
---
...
$$T=\frac{\hat{\beta}_0-b_0}{SE(\hat{\beta}_0)}=\frac{\hat{\beta}_0-b_0}{\hat \sigma\sqrt{({\frac{1}{n}+\frac{\bar{x}^2}{S_{xx}}})}} \sim t_{n-2}$$
...
$$t_{779,1-\frac{\alpha}{2}} \approx z_{1-\frac{\alpha}{2}} =1.96<8.679=T_{test}$$

We confidently reject the null that temperature does not predict bike rides.
...
$$p\text{-value}=2\cdot P(t_{n-2}>|T_{test}|)$$
...
$$P(t_{n-2,\frac{\alpha}{2}}<\frac{\hat{\beta}_1-\beta}{SE(\hat{\beta}_1)}<t_{n-2,1-\frac{\alpha}{2}})=1-\alpha$$

$$CI_{\beta_1}=\left(\hat{\beta}_1-t_{n-2,1-\frac{\alpha}{2}}\underbrace{\frac{\hat \sigma}{\sqrt{S_{xx}}}}_{SE(\hat{\beta}_1)},\hat{\beta}_1+t_{n-2,1-\frac{\alpha}{2}}\underbrace{\frac{\hat \sigma}{\sqrt{S_{xx}}}}_{SE(\hat{\beta}_1)}\right)$$

And Similarly for $\beta_0$

$$CI_{\beta_0}=\left(\hat{\beta}_0-t_{n-2,1-\frac{\alpha}{2}}\underbrace{\hat \sigma\sqrt{({\frac{1}{n}+\frac{\bar{x}^2}{S_{xx}}})}}_{SE(\hat{\beta}_0)},\hat{\beta}_0+t_{n-2,1-\frac{\alpha}{2}}\underbrace{\hat \sigma\sqrt{({\frac{1}{n}+\frac{\bar{x}^2}{S_{xx}}})}}_{SE(\hat{\beta}_0)}\right)$$

<div class="learnr-ref">LearnR: 6.1–6.2 Confidence Intervals (Education & Earnings, Value of Tenure)</div>
...
---
### Confidence Intervals: Pollution Example
...
<div class="learnr-ref">LearnR: 5.1–6.2 Hypothesis Tests & Confidence Intervals</div>
...
$$ (\bar{y}|x=x_0)=\hat{\beta}_0+\hat{\beta}_1x $$
--
What's the expectation?

$$E(\bar{y}|x=x_0)=E(\hat{\beta}_0+\hat{\beta}_1x_0)=\beta_0+\beta_1x_0$$
--

What's the variance?
$$var(\bar{y}|x=x_0)=Var(\hat{\beta}_0+\hat{\beta}_1x_0)=\sigma^2({\frac{1}{n}+\frac{(x_0-\bar{x})^2}{S_{xx}}})$$
...
$$CI_{(\bar{y}|x=x_0)}=\hat{\beta}_0+\hat{\beta}_1x_0 \pm t_{n-2,1-\frac{\alpha}{2}}\underbrace{\hat \sigma\sqrt{({\frac{1}{n}+\frac{(x_0-\bar{x})^2}{S_{xx}}})}}_{SE}$$

<div class="learnr-ref">LearnR: 7.1–7.4 Prediction & Intervals (Healthcare CI, Rent, House Price PI)</div>
...
- $\small \hat{\beta}_0=16892.66$ and  $\small \hat{\beta}_1=723.55$
- n=781
- $\small \bar{x}=16.96$
- $\small S_{xx}=4044$
...
<div class="learnr-ref">LearnR: 7.1–7.2 CI for Mean Response (Healthcare Costs, Average Rent)</div>
...
$$E(\hat{y}|x=x_0)=E(\hat{\beta}_0+\hat{\beta}_1x_0)=\beta_0+\beta_1x_0$$
--

How much true value varies around this prediction?
$$var(y_0-\hat{y}|x=x_0)=Var(\hat{\beta}_0+\hat{\beta}_1x_0)+Var(u_i)=\sigma^2(1+{\frac{1}{n}+\frac{(x_0-\bar{x})^2}{S_{xx}}})$$
...
$$CI_{(\bar{y}|x=x_0)}=\hat{\beta}_0+\hat{\beta}_1x_0 \pm t_{n-2,1-\frac{\alpha}{2}}\underbrace{\hat \sigma\sqrt{(1+{\frac{1}{n}+\frac{(x_0-\bar{x})^2}{S_{xx}}})}}_{SE}$$
...
| OLS estimator | Minimizes $SSE = \sum(y_i - \hat{y}_i)^2$ |
| Slope | $\hat{\beta}_1 = \frac{\widehat{cov}(x,y)}{\widehat{var}(x)}$ |
| $R^2$ | Proportion of variation in $y$ explained by $x$ |
| Hypothesis test | $T = \frac{\hat{\beta}_1 - b_1}{SE(\hat{\beta}_1)} \sim t_{n-2}$ |
| CI for $\beta_1$ | $\hat{\beta}_1 \pm t_{n-2,1-\alpha/2} \cdot SE(\hat{\beta}_1)$ |

--

**Key takeaways:**
- OLS finds the line that minimizes squared residuals
- $\hat{\beta}_1$ tells us the change in $y$ for a one-unit change in $x$
- $R^2$ measures goodness of fit; with one regressor, $R^2 = \rho^2$
- We can test and build CIs for coefficients using the t-distribution
- Mean response CIs are narrower than new response CIs (no individual error)
