...
- Call $\mu_d$ the proportion of Mexican population which has diabetes
  - This is a parameter that you want to learn
...
### Longitudinal Data
- Observations are collected for the same subject over a period of time
...
**Panel data**
-Multiple observation per subject (currency)
...
**Cross-sectional data**
-Single observation per subject (user)
...
**Longitudinal data** 
-Multiple observations of a single subject
...
- Variables that hold numeric data.
  ...
  - **Continuous:** These variables can take any value within a range
      - *Example*: Time (in seconds) Spent on a Webpage
...
- The PDF represents the likelihood of the random variable taking on a specific value within a given range. 
- Unlike the probability mass function (PMF) for discrete variables, the PDF does not give the probability of exact values but provides the relative likelihood of the random variable being within a certain interval.

--

### Example
Suppose we have a random variable X representing the weight of adults in Mexican population. The PDF of X would describe the likelihood of finding a person of a specific weight within a range (e.g., between 58kg and 60kg).
...
The .blue[Cumulative Distribution Function] (CDF) gives the probability that a continuous random variable X will take on a value less than or equal to a specific value x.

For a continuous random variable X with PDF f(x), the CDF F(x) is defined as:

$F(x) = \int_{-\infty}^{x} f(t) \, dt = P(X<x)$

The CDF provides a cumulative view of the probability distribution and is useful for calculating probabilities over intervals and finding percentiles. 
- The CDF starts at 0 as probability of X being less than or equal to negative infinity is 0
- It approaches 1 as x approaches infinity since the the probability of X being less than or equal to positive infinity is 1.
---

## Example 1: Standard Normal

$F(-2) = \int_{-\infty}^{-2} f(t) \, dt = P(X<-2)=0.02$
...
$F(0.2) = \int_{-\infty}^{0.2} f(t) \, dt = P(X<0.2)=0.58$
...
$F(3.2) = \int_{-\infty}^{3.2} f(t) \, dt = P(X<3.2)=0.99$
...
$ECDF(x)=\frac{\sum I(w_i<x)}{N}=\frac{\text{Number of people with weight lower than x}}{N}$
...
```{r example4, echo=FALSE, fig.width=12, fig.height=4, warning=FALSE}
...
$P(weight>100)=1-P(weight<100)=1-ECDF(100)$
