...
```{r 5, out.width='100%', fig.height=3.5 , warning=FALSE}
### Illustration of how variance is sensitive to outliers because squaring amplifies large deviation more than small deviations

library(ggplot2)

# Create a function to calculate squared deviations
calculate_squared_deviations <- function(data, mean_value) {
  deviations <- data - mean_value
  squared_deviations <- deviations^2
  return(squared_deviations)
}

# Create a dataset with more data and an outlier
num_samples <- 1
data_with_outlier <- c(70, 75, 80, 85, 88, 90, 92, 150)
outlier_mean <- mean(data_with_outlier)
outlier_squared_deviations <- calculate_squared_deviations(data_with_outlier, outlier_mean)

# Create a dotplot with data and an outlier
dotplot <- ggplot(data.frame(Value = data_with_outlier), aes(x = Value)) +
  geom_dotplot(binwidth = 1, stackdir = "center", dotsize = 0.5) +
  geom_vline(aes(xintercept = outlier_mean), linetype = "dashed", color = "red") +
  labs(x = NULL, y = NULL) +
  theme_xaringan() +
  theme(legend.position = "none")
...
scatter_plot_data <- data.frame(
  ...
  SquaredDeviation = outlier_squared_deviations
)

# Create a dotplot for squared deviations
scatter_plot <- ggplot(scatter_plot_data, aes(x = Value, y = SquaredDeviation)) +
  geom_point() +
  labs(x = "Observation Value", y = "Sq. Dev.") +
  theme_xaringan()

# Arrange the plots using cowplot
library(cowplot)
plot_grid(scatter_plot, dotplot, ncol = 1)
...
.pull-left[
```{r 5a, out.width='100%', fig.height=3, warning=FALSE}
### table with income by neighborhood
# Create a data frame for the example
neighborhoods <- c("Polanco", "Condesa", "Roma", "Tepito", "Coyoacán", "Santa Fe", "Zona Rosa", "Iztapalapa", "Tacubaya", "San Miguel", "Chalco", "Tláhuac")
average_income <- c(60000, 45000, 35000, 15000, 30000, 25000, 40000, 12000, 18000, 9000, 10000, 12000)
population <- c(10000, 20000, 30000, 5000, 25000, 18000, 12000, 40000, 15000, 8000, 60000, 45000)


# Calculate the unweighted average income
unweighted_mean <- mean(average_income)

# Calculate the weighted average income
weighted_mean <- sum(average_income * population) / sum(population)
df <- data.frame(Neighborhood = neighborhoods,
...
.pull-left[
```{r 5b, out.width='100%', fig.height=3, warning=FALSE}
### table with income by neighborhood
